	...
	globalParams.requestClient(params).catch(function (error) {
		...
		var body = response.data;
	  	if (statusCode != 200) {
			...
			var fuelAutonomy=data.fuelAutonomy;
			var fuelQuantity=data.fuelQuantity;
			if (totalMileage !== undefined) setValue(globalParams.zoe_vin,"totalMileage","number",totalMileage,"data");
			setValue(globalParams.zoe_vin,"fuelAutonomy","number",fuelAutonomy,"data");
			setValue(globalParams.zoe_vin,"fuelQuantity","number",fuelQuantity,"data");
			processNextStep(globalParams, curStep);
		}
	});
	adapter.log.debug("out: " + methodName);
